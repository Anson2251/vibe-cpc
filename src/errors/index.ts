...
export class ErrorRecovery {
     ...
     */
    static findNextStatement(tokens: Token[], currentIndex: number): number {
        ...
        for (let i = currentIndex + 1; i < tokens.length; i++) {
            ...
            if (token.type === TokenType.NEWLINE || token.type === TokenType.EOF_TOKEN) {
                // Check the next token after the newline
                if (i + 1 < tokens.length) {
                    const nextToken = tokens[i + 1];
                    if (
                        typeof nextToken.value === "string" &&
                        statementStarters.includes(nextToken.value)
                    ) {
     ...
     */
    static suggestCorrections(error: SyntaxError, _tokens: Token[]): string[] {
